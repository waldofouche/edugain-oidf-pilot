...
attribute_resolver_file="${script_dir}/shibboleth-idp-docker/shibboleth-idp/conf/attribute-resolver.xml"
attribute_filter_file="${script_dir}/shibboleth-idp-docker/shibboleth-idp/conf/attribute-filter.xml"
...
replace_attribute_resolver="true"
replace_attribute_filter="true"
...
basic_rp_redirect_host="${BASIC_RP_REDIRECT_HOST:-oidc-test.dev.localhost}"
...
basic_rp_test_principal="${BASIC_RP_TEST_PRINCIPAL:-alice}"
...
Flags:
  ...
  --no-replace-attribute-resolver Do not replace attribute-resolver.xml
  --no-replace-attribute-filter Do not replace attribute-filter.xml
...
Optional env vars:
  ...
  BASIC_RP_REDIRECT_HOST=<hostname>   default: oidc-test.dev.localhost
  ...
  BASIC_RP_TEST_PRINCIPAL=<principal> default: alice
EOF
}
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --no-replace-default-rules)
      ...
      ;;
    --no-replace-attribute-resolver)
      replace_attribute_resolver="false"
      ;;
    --no-replace-attribute-filter)
      replace_attribute_filter="false"
      ;;
...
if [[ "${replace_attribute_resolver}" == "true" ]]; then
  cat > "${attribute_resolver_file}" <<'EOF_XML'
<?xml version="1.0" encoding="UTF-8"?>
<AttributeResolver
...
EOF_XML
  echo "Replaced ${attribute_resolver_file}"
  did_mutate="true"
fi

if [[ "${replace_attribute_filter}" == "true" ]]; then
  cat > "${attribute_filter_file}" <<EOF_XML
<?xml version="1.0" encoding="UTF-8"?>
<AttributeFilterPolicyGroup id="ShibbolethFilterPolicy"
    ...
    <AttributeFilterPolicy id="releaseToLocalOIDCRP">
        <PolicyRequirementRule xsi:type="Requester" value="${basic_rp_client_id}" />

        <AttributeRule attributeID="uid" permitAny="true" />
        <AttributeRule attributeID="subject" permitAny="true" />
...
EOF_XML
  echo "Replaced ${attribute_filter_file}"
  did_mutate="true"
fi
...
if [[ "${replace_credentials}" == "false" && "${replace_default_rules}" == "false" && "${replace_attribute_resolver}" == "false" && "${replace_attribute_filter}" == "false" && "${replace_oidc_credentials}" == "false" && "${replace_oidc_clientinfo_resolvers}" == "false" && "${replace_oidc_issuer}" == "false" && "${install_basic_rp}" == "false" && "${replace_relying_party}" == "false" && "${generate_jwks}" == "false" && "${list_installed_modules}" == "false" ]]; then
...
PY
  ...
  echo "Running attribute resolution test for ${basic_rp_test_principal} -> ${basic_rp_client_id}..."
  attribute_doc="$(docker exec "${idp_container}" /opt/shibboleth-idp/bin/aacli.sh -n "${basic_rp_test_principal}" -r "${basic_rp_client_id}")"

  python3 - <<'PY' "${attribute_doc}"
import json
import sys
...
try:
    ...
    raise SystemExit(f"Attribute resolution output is not valid JSON: {e}")

attributes = {
    attr.get("name"): attr.get("values", [])
    for attr in data.get("attributes", [])
}

required = ["uid", "subject", "mail", "eduPersonPrincipalName"]
missing = [name for name in required if not attributes.get(name)]
if missing:
    raise SystemExit("Attribute resolution missing required attributes: " + ", ".join(missing))

print("Attribute resolution validation passed")
print("principal=", data.get("principal"))
print("requester=", data.get("requester"))
